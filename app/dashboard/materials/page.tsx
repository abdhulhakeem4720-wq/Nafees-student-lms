    ...
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-2xl glass-card border border-white/10 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <span className="badge badge-emerald mb-1">Study Material Search Engine</span>
            <h1 className="text-2xl font-bold text-white">Notes, Guides & Worksheets</h1>
            <p className="text-xs text-slate-400">Search & download document files for Grade 6 to 10 Science & Mathematics.</p>
          </div>
      ...
      {filteredMaterials.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {filteredMaterials.map((mat) => (
            <div key={mat.id} className="glass-card p-5 rounded-2xl flex flex-col justify-between border border-white/10">
              <div>
                ...
                <h3 className="font-bold text-base text-white mb-2 line-clamp-2">{mat.title}</h3>
                <p className="text-xs text-slate-300 mb-4">{mat.subjectTitle}</p>
              </div>
              ...
              <div className="pt-4 border-t border-white/10 flex items-center justify-between text-xs">
                ...
                <button
                  ...
                  className="btn-emerald text-xs py-2 px-4 shadow-emerald-500/20"
                >
                  📥 Download Document
                </button>
              </div>
